...
a. Implementar un modulo que almacene informacion de socios de un club participantes de un evento cultural en un arbol binario de busqueda.
De cada socio se debe almacenar numero de socio (entre 100 y 150), nombre y edad (entre 12 y 90). 
La carga finaliza con el numero de socio 100 (que no debe agregarse al arbol) y el arbol debe quedar ordenado por numero de socio.
...
* De cada socio se debe almacenar numero de socio, nombre y edad. La carga finaliza con el numero de socio 0 
* y el arbol debe quedar ordenado por numero de socio. La informacion de cada socio debe generarse aleatoriamente. }
...
begin
 writeln ('(Este de abajo lo hice yo)');
...
procedure InformarExistenciaNombreSocio(a:arbol);
	...
	begin
		if(a<>nil) then
			existeSocio:= (a^.dato.nombre = nombre) or (existeSocio(a^.HI, nombre)) or (existeSocio(a^.HD, nombre))
...
begin
	...
	readln(nombre);
	if(existeSocio(a, nombre))then
		writeln('Sí existe un socio con ese nombre')
	else
		writeln('No existe un socio con ese nombre');
end;
function calcularCantSocios(a: arbol): integer;
	begin
		if(a<>nil) then
			calcularCantSocios:= 1 + calcularCantSocios(a^.HI) + calcularCantSocios(a^.HD)
		else calcularCantSocios:=0;
	end;
procedure InformarCantidadSocios (a: arbol);
begin
	writeln('Cantidad de socios: ', calcularCantSocios(a));
end;
procedure InformarPromedioDeEdad (a: arbol);
	function calcularSumaDeEdades(a:arbol) : integer;
	begin
		if (a<>nil) then
		begin
			calcularSumaDeEdades:= a^.dato.edad + calcularSumaDeEdades(a^.HI) + calcularSumaDeEdades(a^.HD);
		end
		else
			calcularSumaDeEdades:=0;
	end;
begin
	writeln('El promedio de edad entre los socios es: ', (calcularSumaDeEdades(a)/calcularCantSocios(a)):8:2);
end;
var a: arbol; 
Begin
  randomize;
